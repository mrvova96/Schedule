...
public class AdminAddViewModel extends AndroidViewModel {

    private IScheduleRepository repository;
    private MutableLiveData<Boolean> isAdminExists = new MutableLiveData<>();
    private MutableLiveData<Boolean> isAdded = new MutableLiveData<>();
    private CompositeDisposable compositeDisposable = new CompositeDisposable();

    public AdminAddViewModel(@NonNull Application application) {
        super(application);
        repository = new ScheduleRepository(ScheduleDatabase.getInstance(application).scheduleDao());
    }

    public LiveData<Boolean> isAdminExists() {
    ...
    public void checkAdmin(String surname, String name, String patronymic) {
        compositeDisposable.add(repository.isAdminExists(surname, name, patronymic)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Consumer<Boolean>() {
                    @Override
                    public void accept(Boolean isExist) throws Throwable {
                        isAdminExists.setValue(isExist);
                    }
                }));
    }
        ...
        repository.add(loginDetails)
                ...
                .subscribe(new SingleObserver<>() {
                    ...
                    public void onSuccess(@io.reactivex.rxjava3.annotations.NonNull Long ID) {
                        add(new Admin(ID, surname, name, patronymic, facultyID));
                    }
    ...
    private void add(Admin admin) {
        compositeDisposable.add(repository.add(admin)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
